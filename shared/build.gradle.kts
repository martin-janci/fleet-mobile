...
/**
 * The Today standup fixture, shared with claude-fleet.
 *
 * `src/commonTest/fixtures/today-standup.json` is generated by running the
 * desktop's `src/lib/today.ts` itself; the phone's port (`standupText` and
 * `scopeToday` in `model/Today.kt`) must reproduce every expected text in it.
 * See "The standup text is the desktop's" in `skills/fleet-mobile-repo/SKILL.md`.
 *
 * `commonTest` also compiles for Kotlin/Native, where a test cannot read a file
 * from the source tree, so the JSON is wrapped in a generated Kotlin constant
 * and the same assertion runs on the JVM, on the emulator and on the iOS
 * simulator. The JSON file stays the one source: nothing here edits it.
 */
val todayStandupFixture = tasks.register("generateTodayStandupFixture") {
    val source = layout.projectDirectory.file("src/commonTest/fixtures/today-standup.json")
    val outDir = layout.buildDirectory.dir("generated/fixtures/commonTest/kotlin")
    inputs.file(source).withPropertyName("fixture").withPathSensitivity(PathSensitivity.RELATIVE)
    ...
    doLast {
        ...
        val out = outDir.get().file("dev/claudefleet/mobile/model/TodayStandupFixture.kt").asFile
        out.parentFile.mkdirs()
        out.writeText(
            "// Generated by :shared:generateTodayStandupFixture from src/commonTest/fixtures/today-standup.json.\n" +
                "// Do not edit: change the JSON (on the claude-fleet side) instead.\n" +
                "package dev.claudefleet.mobile.model\n\n" +
                "internal const val TODAY_STANDUP_FIXTURE: String = \"\"\"" + escaped + "\"\"\"\n",
        )
    }
}
...
kotlin {
    ...
    sourceSets {
        ...
        commonTest {
            kotlin.srcDir(todayStandupFixture)
        }

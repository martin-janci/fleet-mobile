 ...
 *
 * Anything that is not a [HubError] does **not** get its message repeated. The
 * token-hygiene rule that `HubError` is written around — no wire text reaches a
 * screen, a log or a crash report — is worth nothing if an unexpected throwable
 * can walk its message straight onto the page instead.
 */
